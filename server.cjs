...
const { simpleParser } = require('mailparser'); // EML support
...
app.post('/convert', async (req, res) => {
  try {
    ...
    // JSON mode (and optional debug render)
...
async function renderPdfFromParsed(parsed, opts) {
  ...
  for (const [label, value] of headers) {
    if (!value) continue;
    ({ page, y } = drawWrappedText(pdfDoc, page, font, smallSize, maxTextWidth, pageMargin, y, `${label}: ${value}`, gap, pageMargin));
    y -= 4;
  }
  y -= 8;
  ...
  if (bodyText) {
    ({ page, y } = drawWrappedText(pdfDoc, page, font, bodySize, maxTextWidth, pageMargin, y, bodyText, gap, pageMargin));
    y -= 8;
  }
  ...
  for (const att of inlineImgs) {
    ...
    try {
      ...
      continue; // skip invalid image bytes
    }
    const imgW = embedded.width;
    const imgH = embedded.height;

    // Fit inside text column and within page content height
  ...
  // Merge PDF attachments at end (if any)
  if (mergeAttachments) {
...
function drawWrappedText(pdfDoc, page, font, size, maxWidth, marginX, y, text, gap, pageMargin) {
  const lineHeight = size + gap;
  const color = rgb(0, 0, 0);

  const paragraphs = String(text).replace(/\r/g, '').split(/\n/);
  for (let para of paragraphs) {
    if (!para) { ({ page, y } = ensureRoomFor(page, pdfDoc, y, lineHeight, pageMargin)); y -= lineHeight; continue; }

    const words = para.split(/\s+/);
    let line = '';

    for (let w of words) {
      // Hard-wrap mega tokens (e.g. long URLs)
      if (font.widthOfTextAtSize(w, size) > maxWidth) {
        if (line) ({ page, y } = flushLine(pdfDoc, page, font, size, marginX, y, lineHeight, color, pageMargin, line)), line = '';
        let token = w;
        while (token.length) {
          ...
          ({ page, y } = flushLine(pdfDoc, page, font, size, marginX, y, lineHeight, color, pageMargin, chunk));
      ...
      if (font.widthOfTextAtSize(candidate, size) <= maxWidth) {
        ...
        ({ page, y } = flushLine(pdfDoc, page, font, size, marginX, y, lineHeight, color, pageMargin, line));
        line = w;
      }
    }
    if (line) ({ page, y } = flushLine(pdfDoc, page, font, size, marginX, y, lineHeight, color, pageMargin, line));
    y -= gap; // small gap between paragraphs
  }
  return { page, y };
}

function ensureRoomFor(page, pdfDoc, y, needed, pageMargin) {
...
function flushLine(pdfDoc, page, font, size, marginX, y, lineHeight, color, pageMargin, text = '') {
  ({ page, y } = ensureRoomFor(page, pdfDoc, y, lineHeight, pageMargin));
  page.drawText(text, { x: marginX, y: y - lineHeight, size, font, color });
  y -= lineHeight;
  return { page, y };
}
