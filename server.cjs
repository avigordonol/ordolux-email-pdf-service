...
app.post('/convert', async (req, res) => {
  try {
    ...
    // If JSON requested, optionally test the renderer too (debugRender)
...
async function renderPdfFromParsed(parsed, opts) {
  ...
  // Inline images (cid:...) – scale to fit width/height
  ...
  for (const att of inlineImgs) {
    ...
    try {
      ...
      continue; // skip unknown/invalid image bytes
    }
    const imgW = embedded.width;
    const imgH = embedded.height;

    // Fit inside text column and within remaining page height
    const maxW = maxTextWidth;
    const maxH = (height - pageMargin) - pageMargin; // full content height
  ...
  // Optional: merge attached PDFs at end
  if (mergeAttachments) {
...
function drawWrappedText(pdfDoc, page, font, size, maxWidth, marginX, y, text, gap) {
  ...
  for (let para of paragraphs) {
    ...
    let line = '';
    for (let w of words) {
      // If a single token is too wide (e.g. a long URL), hard-wrap it
      if (font.widthOfTextAtSize(w, size) > maxWidth) {
        // flush current line
        if (line) {
          ({ page, y } = flushLine(page, font, size, marginX, y, lineHeight, color, maxWidth));
          line = '';
        }
        // break the long word
        let token = w;
